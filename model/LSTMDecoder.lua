...
local LSTMDecoder, parent = torch.class('nn.LSTMDecoder', 'nn.Module')
	...
	for L = 1, self.num_layers do
		...
		local new_gate = function()
			...
			return nn.CAddTable(){
				...
				nn.Linear(self.mem_dim, self.mem_dim)(inputs[self.num_layers * 2 + 1 + L])
			}
		end
...
function LSTMDecoder:forward(inputs, word_output, reverse)
	...
	local enc_state = self:tableSelect(inputs, 2, #inputs)
	self.predictions = {}
	...
	for t = 1, size do
		...
		if self.gpu == 0 then label = label:int() end
		...
		local lst = cell:forward(self:get_decoder_input(input, self.rnn_state[t - 1], enc_state, label))
		self.rnn_state[t] = {}
...
function LSTMDecoder:backward(inputs, word_output, reverse)
	...
	for t = size, 1, -1 do		
		...
		local dlst = self.cells[self.depth]:backward(self:get_decoder_input(input, self.rnn_state[t - 1], enc_state, label), self:combine(drnn_state[t], doutput_t))
		drnn_state[t - 1] = {}
...
function LSTMDecoder:get_decoder_input(input, rnn_state, enc_state, label)	
	local resTable={}
	table.insert(resTable, input)
	for _,state in ipairs(rnn_state) do
		table.insert(resTable, state)
	end
	for _,state in ipairs(enc_state) do
		table.insert(resTable, state)
	end
	if self.softmaxtree ==1 then
...
function LSTMDecoder:shareParams(cell, src)
...
function LSTMDecoder:combine(tab, ud)
	local resTable = {}
...
function LSTMDecoder:tableSelect(input, start, last)
